; RUN: opt -passes=safepoint-elimination -S < %s | FileCheck %s

declare hotspotcc void @jeandle.safepoint_poll()

; Test basic counted loop with safepoint
; The loop iterates from 0 to N with step 1

define void @counted_loop_basic(i32 %n) gc "safepoint-in-loop-example" {
entry:
  %cmp = icmp sgt i32 %n, 0
  br i1 %cmp, label %loop.header, label %exit

loop.header:
  %iv = phi i32 [ 0, %entry ], [ %iv.next, %loop.latch ]
  %exit.cond = icmp slt i32 %iv, %n
  br i1 %exit.cond, label %loop.body, label %exit

loop.body:
  call hotspotcc void @jeandle.safepoint_poll()
  
  ; Some loop body work
  %val = load i32, ptr null
  br label %loop.latch

loop.latch:
  %iv.next = add i32 %iv, 1
  br label %loop.header

exit:
  ; Safepoint before ret should NOT be eliminated
  call hotspotcc void @jeandle.safepoint_poll()
  ret void
}

; CHECK-LABEL: @counted_loop_basic(
; CHECK-NOT: call hotspotcc void @jeandle.safepoint_poll()
; CHECK:       exit:
; CHECK-NEXT:    call hotspotcc void @jeandle.safepoint_poll()
; CHECK-NEXT:    ret void
