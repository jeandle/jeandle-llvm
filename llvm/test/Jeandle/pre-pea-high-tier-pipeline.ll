...
; RUN: opt -S --jeandle --jeandle-pea=false --print-pipeline-passes %s 2>&1 | FileCheck %s --check-prefix=PEA-OFF

; The pre-PEA cluster and PEA are gated on PEA being enabled. Frontend
; materialization markers are removed unconditionally after the last round.
...
; PEA-ON: function(post-pea-cleanup),
...
; With PEA disabled there is no pre-PEA cluster or PEA. Marker cleanup
; still precedes the standard post-inline cluster.
; PEA-OFF: {{.*jeandle-inline-driver,}}
; PEA-OFF-NOT: partial-escape-iterative
; PEA-OFF: function(post-pea-cleanup),
; PEA-OFF-NOT: partial-escape-iterative
